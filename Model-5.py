...
# Only drops the row if every single column is NaN
gdf_in_ucmr_model_E = gdf_in_ucmr_model_E.dropna()
# gdf_in_ucmr_model_E.dropna(inplace=True)
X_with_labels = gdf_in_ucmr_model_E.drop(columns=[ 'HUC8', 'geometry','Industry_Count','NAME','density', 'violation_status', 'Mean_SVI']) 
...
class ModelEvaluationUI:
    def __init__(self, root, X, y, feature_names, naics_map):
        ...
        self.model_selector['values'] = ("RandomForest", "LogisticRegression", "MLP", "kNN")
    ...
    def run_pipeline(self):
        ...
        elif m_type == "kNN": self.current_model = KNeighborsClassifier(n_neighbors=5)
